...
const JWT_SECRET = 'kamalisgood$boy'
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ error: 'Please try to login with coreect credentials' })
      }
...
// 2 =Get  Login a User details: POST "/api/auth/getuser". Login required
